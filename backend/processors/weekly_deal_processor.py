...
{{
  "index": <numéro de l'article choisi>,
  "product": "nom du produit en hébreu ou français",
  "price": "prix ou réduction (ex: 9.90₪, -30%)",
  "description": "une phrase décrivant l'offre"
}}

Si aucune promotion claire n'est trouvée, réponds : {{"index": null, "product": null, "price": null, "description": null}}"""
...
- Titre : "🛒 Promotions de la semaine — AL.IA a trouvé pour vous !"
...
- Termine par : "💬 Parle à AL.IA 👉 https://wa.me/972549675013?text=Avant%20de%20commencer%2C%20pr%C3%A9sente-toi."
...
- Для каждого супермаркета включи прямую ссылку на товар, если она есть, иначе общую :
  ...
  • Rami Levy : {link_rami}
  • Hazi Hinam : {link_carrefour}
- Заверши : "💬 Напиши AL.IA 👉 https://wa.me/972549675013?text=%D0%9F%D1%80%D0%B5%D0%B4%D1%81%D1%82%D0%B0%D0%B2%D1%8C%D1%81%D1%8F."
...
async def _pick_best_deal(supermarket_name: str, items: list[dict], db_col: str = "") -> dict:
    ...
    candidates = items[:15]
    items_text = "\n".join(f"{i}. {item['text']}" for i, item in enumerate(candidates))
...
async def generate_weekly_deals(raw_data: dict | None = None, force: bool = False) -> dict:
    ...
    # Use product-specific URLs when available, fallback to general promo pages
    link_shufersal = shufersal_deal.get("url") or SUPERMARKET_LINKS["shufersal"]
    link_rami = rami_deal.get("url") or SUPERMARKET_LINKS["rami_levy"]
    link_carrefour = hazi_hinam_deal.get("url") or SUPERMARKET_LINKS["carrefour"]
    ...
    fr_response, ru_response = await asyncio.gather(
        client.messages.create(
            ...
            messages=[{"role": "user", "content": COMBINED_FR_PROMPT.format(
                ...
                link_shufersal=link_shufersal,
                link_rami=link_rami,
                link_carrefour=link_carrefour,
            )}],
        ),
        client.messages.create(
            model="claude-haiku-4-5-20251001",
