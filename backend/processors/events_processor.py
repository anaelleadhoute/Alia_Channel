...
- Sélectionne 5-7 événements en priorité : concerts, spectacles, événements culturels, gastronomie, festivals.
- OBLIGATOIRE : inclus au moins 1 événement dont la source est "Meetup", "Eventbrite" ou "Mairie de Tel Aviv" (pas uniquement Secret Tel Aviv).
- OBLIGATOIRE : inclus une section "👨‍👩‍👧 Pour les familles" avec au moins 1 événement adapté aux enfants/familles (spectacles jeunesse, ateliers, sorties famille, etc.)
- Pour chaque événement : nom, ville, date/heure, et le lien exact fourni (ne modifie pas les URLs)
...
- Выбери 5-7 событий в приоритете : концерты, спектакли, культурные мероприятия, гастрономия, фестивали.
- ОБЯЗАТЕЛЬНО : включи минимум 1 событие с источником "Meetup", "Eventbrite" или "Mairie de Tel Aviv" (не только Secret Tel Aviv).
- ОБЯЗАТЕЛЬНО : включи раздел "👨‍👩‍👧 Для семей" с минимум 1 событием для детей/семей (детские спектакли, мастер-классы, семейные прогулки и т.д.)
- Для каждого события : название, город, дата/время и точная ссылка из данных (не изменяй URL)
