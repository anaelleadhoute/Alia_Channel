...
Voici des activités pour enfants et familles cette semaine :
{events_text}
...
[2-3 événements/activités avec nom, date si disponible, lien exact]

💡 Idée activité de la semaine : [reprends l'activité dont la source est "Karamel" — présente-la comme une idée à faire avec les enfants, avec son lien]

💬 Parle à Alia 👉 https://wa.me/972549675013?text=Avant%20de%20commencer%2C%20pr%C3%A9sente-toi.

Ne modifie pas les URLs. Ton chaleureux et enthousiaste. 100-140 mots.
Réponds uniquement avec le texte du message."""

KIDS_RU_PROMPT = """Ты редактор Alia Channel — медиа для русскоязычных олим в Израиле.

Вот детские и семейные мероприятия этой недели :
{events_text}
...
💡 Идея на неделю : [используй активность с источником "Karamel" — представь как идею для детей, со ссылкой]

💬 Напиши Alia 👉 https://wa.me/972549675013?text=%D0%9F%D1%80%D0%B5%D0%B4%D1%81%D1%82%D0%B0%D0%B2%D1%8C%D1%81%D1%8F.

Не меняй URL. Тёплый и живой тон. 100-140 слов.
...
async def generate_weekly_kids_events(force: bool = False, raw_events: list[dict] | None = None) -> dict:
    ...
    fr_resp, ru_resp = await asyncio.gather(
        claude.messages.create(model="claude-haiku-4-5-20251001", max_tokens=400,
